...
export default function SpeakingStitch4Page() {
  ...
  function truncateFeedback(text: string): string {
    const maxLength = 140; // Approximately 2 lines at small text size
    ...
    if (lastPunctuation > maxLength * 0.7) { // If we find punctuation in the last 30% of text
  ...
  return (
    <>
      ...
      <div className="flex h-screen flex-col bg-[var(--background-dark)] text-[var(--text-primary)]" style={{fontFamily: '"Space Grotesk", "Noto Sans", sans-serif'}}>
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-[var(--border-dark)] px-10 py-3">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-bold leading-tight tracking-[-0.015em]"><Link href="/dashboard">LevantTalk</Link></h2>
          </div>
          <nav className="flex items-center gap-9">
            <a className="text-white/80 hover:text-white text-sm font-medium leading-normal transition-colors" href="#">Home</a>
            <a className="text-white/80 hover:text-white text-sm font-medium leading-normal transition-colors" href="#">Lessons</a>
          ...
          <div className="flex gap-2">
            <button className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-md h-10 px-4 bg-[var(--accent-blue)] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[var(--accent-blue-hover)] transition-colors">
              <span className="truncate">Sign up</span>
            </button>
            <button className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-md h-10 px-4 bg-[var(--border-dark)] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#2c4c5c] transition-colors">
        ...
        <div className="flex-shrink-0 py-4">
          <div className="container mx-auto flex items-center justify-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="w-16 h-10 rounded-md bg-[var(--surface-dark)] border border-[var(--border-dark)] flex items-center justify-center text-sm font-medium text-white/70">1</div>
              <div className="w-16 h-10 rounded-md bg-[var(--accent-blue)] border border-[var(--accent-blue)] flex items-center justify-center text-sm font-bold text-white">2</div>
              <div className="w-16 h-10 rounded-md bg-[var(--surface-dark)] border border-[var(--border-dark)] flex items-center justify-center text-sm font-medium text-white/70">3</div>
              <div className="w-16 h-10 rounded-md bg-[var(--surface-dark)] border border-[var(--border-dark)] flex items-center justify-center text-sm font-medium text-white/70">4</div>
              <div className="w-16 h-10 rounded-md bg-[var(--surface-dark)] border border-[var(--border-dark)] flex items-center justify-center text-sm font-medium text-white/70">5</div>
              <div className="w-16 h-10 rounded-md bg-[var(--surface-dark)] border border-[var(--border-dark)] flex items-center justify-center text-sm font-medium text-white/70">...</div>
              <div className="w-16 h-10 rounded-md bg-[var(--surface-dark)] border border-[var(--border-dark)] flex items-center justify-center text-sm font-medium text-white/70">10</div>
            </div>
            <button className="text-white/50 hover:text-white transition-colors">
        ...
        <main className="flex flex-1 p-8 pt-0 gap-8 overflow-hidden">
          {exercise ? (
            <>
              <div className="flex flex-col w-1/2 bg-[var(--surface-dark)] rounded-xl p-8 justify-between">
            <div>
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2 bg-[#2c4c5c] text-white px-3 py-1 rounded-full text-sm font-medium">
                  ...
                  <span>{exercise ? getDifficultyLabel(exercise.difficulty_level) : "Intermediate"}</span>
                </div>
                <div className="text-[var(--text-secondary)] text-sm font-medium">Attempt {attemptCount > 0 ? `${attemptCount}/5` : "1/5"}</div>
              </div>
              
              {!editing ? (
                <div className="relative group">
                  <p className="text-white text-2xl font-normal leading-loose text-right" style={{fontFamily: '"Noto Sans Arabic", sans-serif'}}>
                    {exercise?.arabic_text || "الطقس جميل اليوم. الشمس مشرقة والسماء صافية. إنه يوم مثالي للذهاب في نزهة في الحديقة أو لتناول وجبة مع الأصدقاء. الطيور تغني والأزهار تتفتح. إنه يوم رائع للتواجد في الخارج والاستمتاع بالطبيعة."}
                  </p>
            ...
            <div className="w-full flex flex-col items-center gap-4">
              <audio 
                ...
                onEnded={() => {
                  setAudioPlaying(false);
                  const el = audioRef.current;
                  if (el && el.duration) setAudioTime(el.duration);
                }}
                onError={() => {
                  setAudioDuration(0);
                  setAudioTime(0);
              ...
              <div className="w-full flex items-center gap-4 text-[var(--text-secondary)] text-sm font-medium">
                ...
                <div 
                  className="relative flex-1 h-1.5 bg-[#315668] rounded-full cursor-pointer"
                  onClick={(e) => {
                    e.preventDefault();
                    const el = audioRef.current;
                    if (!el || !audioDuration || audioDuration <= 0) return;
                ...
                >
                  <div 
                    className="absolute h-full bg-[var(--accent-blue)] rounded-full"
                    style={{ width: audioDuration ? `${(audioTime / audioDuration) * 100}%` : "0%" }}
                  ></div>
                  <div 
                    className="absolute w-4 h-4 bg-white rounded-full -top-1.5 cursor-pointer hover:scale-110 transition-transform"
              ...
              <button className="flex items-center justify-center gap-3 h-12 w-12 bg-red-600 hover:bg-red-700 transition-colors text-white rounded-full"
            ...
            <div className="flex flex-col w-1/2 bg-[var(--surface-dark)] rounded-xl p-6 gap-4">
            ...
            <div className="flex justify-center items-center mb-4 relative">
              <button 
                className={`flex items-center justify-center gap-3 h-16 w-16 rounded-full transition-colors text-white shadow-[0_0_0_8px_rgba(13,166,242,0.3)] ${
                  recording ? "bg-red-600 hover:bg-red-500" : "bg-[var(--accent-blue)] hover:bg-[var(--accent-blue-hover)]"
                }`}
              ...
              {recording && (
                <div className="absolute left-1/2 translate-x-12 text-white/70 text-xs font-medium bg-black/20 px-2 py-1 rounded-md whitespace-nowrap">
            ...
            <div className="bg-[#101d23] rounded-lg p-4 border border-[var(--border-dark)]">
              ...
              <div className="text-[var(--text-secondary)] text-xs mb-3 h-8 overflow-hidden">
                <p className="font-medium text-white mb-1 line-clamp-2">
                  {score !== null ? 
                    (feedbackMessage || "Great job! Keep practicing to improve your pronunciation.") : 
                    "Ready to start? Press the microphone button and speak clearly. Let's see how well you can pronounce the Arabic text!"
                  }
                </p>
              </div>
              <div className="flex items-center gap-2 pt-2 mt-2 border-t border-[var(--border-dark)]">
                <button className="flex-1 h-8 px-3 rounded-md bg-[var(--border-dark)] text-white text-xs font-bold hover:bg-[#2c4c5c] transition-colors"
                  onClick={startRecording}>
                  <span className="truncate">{score !== null ? "Try Again" : "Start Recording"}</span>
                </button>
                {score !== null && score >= 60 && (
                  <button className="flex-1 h-8 px-3 rounded-md bg-[var(--accent-blue)] text-white text-xs font-bold hover:bg-[var(--accent-blue-hover)] transition-colors"
                    onClick={fetchExercise}>
                    <span className="truncate">Next Challenge</span>
                  </button>
                )}
              </div>
              </div>
            ...
            <div className="flex items-center justify-center w-full h-full">
              <div className="text-white/60 text-lg">Loading exercise...</div>
            </div>
          )}
        </main>
      </div>
