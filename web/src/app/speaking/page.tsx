...
export default function SpeakingPage() {
  ...
  const goPrev = () => { if (!canPrev) return; const prev = library[idx - 1]; setExercise(prev); };
      ...
      <div className="flex min-h-screen flex-col bg-[var(--background-dark)] text-[var(--text-primary)]" style={{fontFamily:'"Space Grotesk", "Noto Sans", sans-serif'}}>
        ...
        <main className="flex-1 max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          {exercise ? (
              ...
              <div className="flex flex-col bg-[var(--surface-dark)] rounded-xl p-6 gap-4">
                ...
                {!editingExercise ? (
                  <div className="relative group">
                    ...
                    {(role === "teacher" || role === "admin") && (
                      <button className="absolute top-0 right-0 text-white/60 hover:text-white opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => setEditingExercise(true)} title="Edit text">✎</button>
                    )}
                  </div>
                ) : (
            ...
            <div className="flex items-center justify-center w-full h-full text-white/60">No exercise found</div>
          )}
        </main>
      </div>
    </>
